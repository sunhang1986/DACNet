...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points = tf_util_srn.conv2d(new_points, num_out_channel, [1,1],
                                        padding='SAME', stride=[1,1],
                                        bn=bn, is_training=is_training,
                                        scope='conv%d'%(i), bn_decay=bn_decay)

        if pooling=='avg':
            new_points = tf_util_srn.avg_pool2d(new_points, [1,nsample], stride=[1,1], padding='VALID', scope='avgpool1')
            ...
            new_points = tf_util_srn.max_pool2d(-1*new_points, [1,nsample], stride=[1,1], padding='VALID', scope='minpool1')
        elif pooling=='max_and_avg':
            avg_points = tf_util_srn.max_pool2d(new_points, [1,nsample], stride=[1,1], padding='VALID', scope='maxpool1')
            max_points = tf_util_srn.avg_pool2d(new_points, [1,nsample], stride=[1,1], padding='VALID', scope='avgpool1')
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            for j,num_out_channel in enumerate(mlp_list[i]):
                ...
                grouped_points = tf_util_srn.conv2d(grouped_points, num_out_channel, [1,3],
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, scope, bn=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points1 = tf_util_srn.conv2d(new_points1, num_out_channel, [1,1],
                                         padding='VALID', stride=[1,1],
                                         bn=bn, is_training=is_training,
